...
#    define _TRINITY_REALM_CONFIG "queueserver.conf"
#endif
#ifndef _TRINITY_REALM_CONFIG_DIR
#    define _TRINITY_REALM_CONFIG_DIR "queueserver.conf.d"
#endif
...
int main(int argc, char** argv)
{
    ...
    Trinity::Banner::Show(
        "queueserver", [](char const* text) { TC_LOG_INFO("server.queueserver", "{}", text); },
        []()
        {
            TC_LOG_INFO("server.queueserver", "Using configuration file {}.", sConfigMgr->GetFilename());
            TC_LOG_INFO("server.queueserver", "Using SSL version: {} (library: {})", OPENSSL_VERSION_TEXT,
                        OpenSSL_version(OPENSSL_VERSION));
            TC_LOG_INFO("server.queueserver", "Using Boost version: {}.{}.{}", BOOST_VERSION / 100000,
                        BOOST_VERSION / 100 % 1000, BOOST_VERSION % 100);
        });

    for (std::string const& key : overriddenKeys)
        TC_LOG_INFO("server.queueserver", "Configuration field '{}' was overridden with environment variable.", key);
    ...
    if (!sQueueSocketMgr.StartNetwork(*ioContext, bindIp, port))
    {
        TC_LOG_ERROR("server.queueserver", "Failed to initialize network");
        return 1;
    }

    std::shared_ptr<void> sQueueSocketMgrHandle(nullptr, [](void*) { sQueueSocketMgr.StopNetwork(); });
...
bool StartDB()
{
    ...
    // Load databases
    DatabaseLoader loader("server.queueserver", DatabaseLoader::DATABASE_NONE);
                              ...
                              boost::system::error_code const& error)
{
    if (!error)
    {
        ...
        {
            TC_LOG_INFO("server.queueserver", "Ping MySQL to keep connection alive");
