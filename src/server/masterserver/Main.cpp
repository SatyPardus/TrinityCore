...
#    define _TRINITY_REALM_CONFIG "masterserver.conf"
#endif
#ifndef _TRINITY_REALM_CONFIG_DIR
#    define _TRINITY_REALM_CONFIG_DIR "masterserver.conf.d"
#endif

#if TRINITY_PLATFORM == TRINITY_PLATFORM_WINDOWS
#include "ServiceWin32.h"
char serviceName[]        = "masterserver";
char serviceLongName[]    = "TrinityCore master service";
char serviceDescription[] = "TrinityCore World of Warcraft emulator master service";
/*
...
bool StartDB();
void StopDB();
void SignalHandler(std::weak_ptr<Trinity::Asio::IoContext> ioContextRef, boost::system::error_code const& error,
                   int signalNumber);
...
int main(int argc, char** argv)
{
    ...
    bool additionalConfigFileLoadSuccess =
    ...
    Trinity::Banner::Show(
        "masterserver", [](char const* text) { TC_LOG_INFO("server.masterserver", "{}", text); },
        []()
        {
            TC_LOG_INFO("server.masterserver", "Using configuration file {}.", sConfigMgr->GetFilename());
            TC_LOG_INFO("server.masterserver", "Using SSL version: {} (library: {})", OPENSSL_VERSION_TEXT,
                        OpenSSL_version(OPENSSL_VERSION));
            TC_LOG_INFO("server.masterserver", "Using Boost version: {}.{}.{}", BOOST_VERSION / 100000,
                        BOOST_VERSION / 100 % 1000, BOOST_VERSION % 100);
        });

    for (std::string const& key : overriddenKeys)
        TC_LOG_INFO("server.masterserver", "Configuration field '{}' was overridden with environment variable.", key);
    ...
    if (!sMasterSocketMgr.StartNetwork(*ioContext, bindIp, port))
    {
        TC_LOG_ERROR("server.masterserver", "Failed to initialize network");
        return 1;
    }

    std::shared_ptr<void> sMasterSocketMgrHandle(nullptr, [](void*) { sMasterSocketMgr.StopNetwork(); });
...
#endif
    signals.async_wait(std::bind(&SignalHandler, std::weak_ptr<Trinity::Asio::IoContext>(ioContext),
                                 std::placeholders::_1, std::placeholders::_2));

    // Set process priority according to configuration settings
    SetProcessPriority("server.masterserver", sConfigMgr->GetIntDefault(CONFIG_PROCESSOR_AFFINITY, 0),
    ...
    dbPingTimer->async_wait(std::bind(&KeepDatabaseAliveHandler,
                                      std::weak_ptr<Trinity::Asio::DeadlineTimer>(dbPingTimer), dbPingInterval,
                                      std::placeholders::_1));
    ...
    if (m_ServiceStatus != -1)
    {
        ...
        serviceStatusWatchTimer->async_wait(
            std::bind(&ServiceStatusWatcher, std::weak_ptr<Trinity::Asio::DeadlineTimer>(serviceStatusWatchTimer),
                      std::weak_ptr<Trinity::Asio::IoContext>(ioContext), std::placeholders::_1));
    }
#endif
    ...
    TC_LOG_INFO("server.masterserver", "Halting process...");
...
bool StartDB()
{
    ...
    // Load databases
    DatabaseLoader loader("server.masterserver", DatabaseLoader::DATABASE_NONE);
    loader
    ...
    TC_LOG_INFO("server.masterserver", "Started master database connection pool.");
    return true;
}
                              ...
                              boost::system::error_code const& error)
{
    if (!error)
    {
        ...
        {
            TC_LOG_INFO("server.masterserver", "Ping MySQL to keep connection alive");
    ...
    if (!error)
    {
        if (std::shared_ptr<Trinity::Asio::IoContext> ioContext = ioContextRef.lock())
        {
            if (m_ServiceStatus == 0)
                ioContext->stop();
            else if (std::shared_ptr<Trinity::Asio::DeadlineTimer> serviceStatusWatchTimer =
                         serviceStatusWatchTimerRef.lock())
            {
                serviceStatusWatchTimer->expires_from_now(boost::posix_time::seconds(1));
                serviceStatusWatchTimer->async_wait(
...
variables_map GetConsoleArguments(int argc, char** argv, fs::path& configFile, fs::path& configDir,
                                  [[maybe_unused]] std::string& winServiceAction)
{
    ...
    win.add_options()("service,s", value<std::string>(&winServiceAction)->default_value(""),
